...
cli({
  site: 'suno',
  name: 'download-ui',
  description: '下载 Suno 歌曲（MP3/WAV/M4A），优先从 API 获取真实 URL，失败时回退到 CDN',
  access: 'write',
  ...
  func: async (page, args) => {
    ...
    // ── 1. Get clip details via browser fetch ──
    ...
    // ── 2. Download cover via browser fetch ──
    let coverFile = '';
    if (coverUrl) {
      ...
      if (coverResult.ok) {
        ...
        console.warn('[download-ui] Cover download failed: ' + (coverResult.status || coverResult.err));
        coverFile = '';
      }
    }
    ...
    // If m4a is requested, try media_urls first (direct CDN, no UI needed)
    if (format === 'm4a') {
    ...
    if (format === 'wav' && !audioFile) {
      ...
      if (!audioFile) {
        ...
        if (wavResult.ok) {
          ...
          console.warn('[download-ui] WAV CDN download failed: ' + (wavResult.status || wavResult.err) + ', falling back to MP3');
        }
      }
    }
